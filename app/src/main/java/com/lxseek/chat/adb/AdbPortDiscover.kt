...
private const val TAG = "AdbDns"

/**
 * Discovers the local ADB wireless-debugging port via mDNS (NSD).
 *
 * Ported from LADB's DnsDiscover: scans for `_adb-tls-connect._tcp` services on the
 * local network and picks the one with the newest expiration time / highest numbered
 * service name (e.g. "ADB (3)" beats "ADB (2)").
 */
class AdbPortDiscover private constructor(
    ...
    companion object {
        private var instance: AdbPortDiscover? = null
        var adbPort: Int? = null
        var pendingResolves = AtomicBoolean(false)
        var aliveTime: Long? = null

        fun getInstance(context: Context, nsdManager: NsdManager): AdbPortDiscover {
            return instance ?: AdbPortDiscover(context, nsdManager).also { instance = it }
        }

        /** Resets the singleton state so a fresh scan can pick a new port. */
        fun reset() {
            instance = null
            adbPort = null
