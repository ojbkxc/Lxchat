...
class AutoBackupManager(
    ...
    private suspend fun performBackup(): File? = withContext(Dispatchers.IO) {
        try {
            ...
            val filename = "LxChat_backup_${sdf.format(Date())}.lxchat"
    ...
    private suspend fun resolveBackupDir(): File? {
        val stored = settingsManager.autoBackupDirectory.safeRead("Download/LxChat/Backup")
    ...
    private suspend fun cleanupOldBackups() {
        ...
        dir.listFiles { f ->
            f.isFile && f.name.startsWith("LxChat_backup_") && f.name.endsWith(".lxchat")
        }?.forEach { file ->
        ...
        dir.listFiles { f ->
            f.isFile && f.name.startsWith("LxChat_backup_") && f.name.endsWith(".lxchat.tmp")
        }?.forEach { tmpFile ->
