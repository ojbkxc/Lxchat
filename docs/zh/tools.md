...
LxChat 的模型可以自主使用工具——它们决定搜索什么、执行什么、读取什么或记住什么，无需你手动触发每个操作。工具在**多轮循环**中运作：模型可以调用一个工具、读取结果、然后决定调用另一个工具或回复。
...
通过 [Conch](https://github.com/ojbkxc/conch) 协议在远程机器上执行命令。模型可以检查服务器状态、管理文件或运行脚本。
