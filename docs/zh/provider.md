...
!!! warning
    API 密钥存储在加密的 Room 数据库中。它们永远不会发送到 LxChat 服务器（LxChat 没有服务器）。但如果包含在 `.lxchat` 导出文件中，它们会以明文形式导出。
...
LxChat 从 `{base_url}/v1/models` 获取模型列表。添加后，自定义提供商与内置提供商完全一样使用：添加 API 密钥、同步模型、聊天。
