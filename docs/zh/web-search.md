...
LxChat 使用 Kagi 当前的 v1 Search API。搜索费用由 Kagi 按 API 账户用量计费。
...
- 你的查询发送给搜索提供商（Brave、Serper 等），而非 LxChat
- LxChat 不记录或存储你的搜索查询（除了在对话本身中）
