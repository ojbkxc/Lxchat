...
LxChat 支持 GGUF 格式的聊天和嵌入模型。聊天模型需要能装入设备内存（1–8B 参数量，取决于 RAM）。嵌入模型更小（100–500 MB）。详见 [本地模型](local-model.md)。
...
3. 在 LxChat 中，用 **Ollama** 预设添加远程嵌入模型
...
是的。Conch 使用 ECDH 密钥交换 + AES-256-GCM 加密 + HMAC-SHA256 签名。LxChat 与 Conch 服务器之间的所有流量均为端到端加密。
...
创建手动 `.lxchat` 备份。如需免手动保护，在 设置 → 数据控制 → 自动备份 中启用**自动备份** — 它会在后台定期备份你的数据。详见 [数据迁移](import-export.md)。
...
可选。导出界面允许你切换是否包含 API 密钥。启用时会有警告提示。密钥以明文形式存储在 `.lxchat` 文件中，因此仅在完全设备迁移且信任目标时包含。
...
所有数据存储在 Android 设备的 Room 数据库中。LxChat 没有服务器、没有云同步、没有遥测。消息直接从你的设备发送到你配置的 AI 提供商。
...
在 [GitHub](https://github.com/ojbkxc/lxchat/issues) 上提交 issue。贡献方式见 README 的 [Contributing](https://github.com/ojbkxc/lxchat#contributing) 部分。
