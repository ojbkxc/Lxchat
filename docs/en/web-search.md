...
1. In LxChat, go to **Settings → Web Search**
...
2. In LxChat, go to **Settings → Web Search**
...
LxChat uses Kagi's current v1 Search API. Searches are billed by Kagi according to your API account.
...
- Your queries go to the search provider (Brave, Serper, etc.), not to LxChat
- LxChat does not log or store your search queries (except in the conversation itself)
