...
LxChat supports GGUF format for both chat and embedding. Chat models should fit in device memory (1–8B parameters depending on RAM). Embedding models are much smaller (100–500 MB). See [Local Models](local-model.md).
...
3. In LxChat, add a remote embedding model with the **Ollama** preset
...
Yes. Conch uses ECDH key exchange + AES-256-GCM encryption + HMAC-SHA256 signing. All traffic between LxChat and the Conch server is end-to-end encrypted.
...
Go to Settings → Data Control → **Export Data** to create a manual `.lxchat` backup. For hands-off protection, enable **Auto Backup** in Settings → Data Control → Auto Backup — it periodically backs up your data in the background. See [Data Portability](import-export.md).
...
They can be, but it's optional. The export screen lets you toggle API key inclusion. A warning is shown when you enable it. Keys are stored in plain text within the `.lxchat` file, so only include them for full device migrations to trusted destinations.
...
Everything is stored locally on your Android device in a Room database. LxChat has no servers, no cloud sync, no telemetry. Messages are sent directly from your device to the AI provider you configure.

### Does LxChat support multiple languages?
...
Open an issue on [GitHub](https://github.com/ojbkxc/lxchat/issues). For contributions, see the [Contributing](https://github.com/ojbkxc/lxchat#contributing) section of the README.
