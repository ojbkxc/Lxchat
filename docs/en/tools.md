...
LxChat's models can autonomously use tools — they decide what to search, execute, read, or remember without you needing to manually trigger each action. Tools operate in **multi-round loops**: the model can call a tool, read the result, then decide to call another tool or respond.
...
4. LxChat executes the tool on-device or on a remote server
...
Execute commands on remote machines via the [Conch](https://github.com/ojbkxc/conch) protocol. The model can check server status, manage files, or run scripts.
