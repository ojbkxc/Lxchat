...
LxChat stores all your data on-device and provides full import/export capabilities. You own your data — move it in, move it out, back it up.

## Export

Export your data to a single `.lxchat` file — a portable archive that contains everything LxChat stores.
...
!!! danger "API Keys Warning"
    API keys are exported in **plain text**. Anyone with the `.lxchat` file can read your keys. Only enable API key export if you trust the destination and handle the file securely.
...
4. Tap **Export**
5. Choose where to save the `.lxchat` file

---

## Import

Restore data from a previous `.lxchat` export.

### Import Strategies

When importing, you choose how LxChat handles data that already exists on your device:
...
1. Go to **Settings → Data Control**
2. Tap **Import Data**
3. Select a `.lxchat` file
4. Review the import preview — see what's in the file (export date, version, content counts)
5. Choose an import strategy
6. Tap **Import**

!!! danger "API Keys Warning"
    If the export file contains API keys, LxChat warns you before importing. Keys are imported in plain text. Only proceed if you trust the source of the file.
...
Both Claude and ChatGPT export your data as a **`.zip` archive**. LxChat imports that `.zip` directly — there is no need to unzip it first, and LxChat does **not** accept loose `.json` files.
...
!!! note
    LxChat reads the conversation data straight out of Claude's `.zip` export. Attachments are detected and shown in the preview, but only the message text is imported — attachment files themselves are not.
...
The `.lxchat` file is a JSON-based archive. If you're technically inclined, you can inspect or process it with standard tools. The format is designed for forward and backward compatibility.

---

## Auto Backup

LxChat can automatically back up your data on a schedule. You don't need to remember to export — LxChat handles it for you.

### How It Works

- Auto backup runs periodically in the background using Android WorkManager
- When a backup is due, LxChat exports your selected categories to the configured directory
...
5. Set **Backup location** — tap to pick a folder (defaults to `Download/LxChat/Backup`)
