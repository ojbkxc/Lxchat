...
LxChat bundles llama.cpp via Android NDK (CMake). When you import a GGUF file, the model runs entirely on your device's CPU — no data leaves the device.
...
Long-press a model and tap **Delete**. This removes the model from LxChat and deletes the GGUF file from storage.
