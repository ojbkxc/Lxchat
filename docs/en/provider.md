...
LxChat connects directly to AI providers — no middleman, no subscription, no telemetry. You bring your own API keys, and everything runs from your device.
...
!!! warning
    API keys are stored locally in an encrypted Room database. They are never sent to LxChat servers (there are none). However, they are exported in plain text if you include them in a `.lxchat` export file.
...
LxChat fetches the model list from `{base_url}/v1/models`. Once added, custom providers work exactly like built-in ones: add API keys, sync models, and chat.
...
3. LxChat fetches available models from every configured provider
