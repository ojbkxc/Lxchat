...
System prompts define the model's persona, behavior, and ground rules. LxChat gives you fine-grained control over how instructions are assembled and sent to the model.
...
**Example**: If your prefix is `[Context: working on LxChat docs]` and suffix is `\n\nPlease reply in Markdown.`, the model receives:

```text
[Context: working on LxChat docs]
...
LxChat ships with a library of ready-to-use prompt templates organized into 4 categories:
...
LxChat can automatically generate conversation titles after the first response:
