...
Route all of LxChat's network traffic through an HTTP or SOCKS proxy. This is useful on restricted networks, for routing requests through a specific gateway, or when a provider is only reachable via a proxy.
