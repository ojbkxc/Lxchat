...
Embedding models convert text into numerical vectors that capture meaning. LxChat uses these vectors for semantic search (RAG) over your conversation history — finding messages by what they mean, not just what words they contain.
...
4. LxChat computes **cosine similarity** between the query vector and all message vectors
...
!!! tip
    The API key field is optional if you've already configured the same provider for chat. Leave it blank and LxChat resolves your chat API key automatically.
...
3. In LxChat, add a remote model:
    - Provider: **Ollama**
...
2. LxChat processes all uncached messages in batches
...
When you add a remote model, LxChat runs an automatic connection test. If it fails:
