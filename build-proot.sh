#!/usr/bin/env bash
set -euo pipefail
# build-proot.sh - Build proot native binaries for the LxChat Android app.
...
# PRoot .so are fdroid-only (PRoot sandbox is fdroid flavor exclusive). They
# are deployed directly to app/src/fdroid/jniLibs/ and never to main/jniLibs,
# so the play flavor APK does not ship PRoot runtime binaries.
...
elif [ ! -f "$FDROID_JNILIBS/libproot_exec.so" ] || \
     [ ! -f "$FDROID_JNILIBS/libproot_loader.so" ] || \
     [ ! -f "$FDROID_JNILIBS/libtalloc.so" ]; then
    need_rebuild=true
...
"$STRIP" --strip-all \
    "$BLD_DIR/src/proot" \
    -o "$FDROID_JNILIBS/libproot_exec.so"
...
cp "$LOADER_SRC" "$FDROID_JNILIBS/libproot_loader.so"

# talloc -> jniLibs (strip)
"$STRIP" --strip-all \
    "$SYSROOT_LIB/libtalloc.so" \
    -o "$FDROID_JNILIBS/libtalloc.so"

echo "  [3/3] Binaries deployed:"
echo "    $(stat -c%s "$FDROID_JNILIBS/libproot_exec.so") bytes  libproot_exec.so"
echo "    $(stat -c%s "$FDROID_JNILIBS/libproot_loader.so") bytes  libproot_loader.so"
echo "    $(stat -c%s "$FDROID_JNILIBS/libtalloc.so") bytes  libtalloc.so"
